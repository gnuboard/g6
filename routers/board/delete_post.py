    ...
    def delete_write(self):
        ...
        if self.admin_type != "super":
            if self.admin_type and write_member_level > self.member_level:
                raise ClassException(status_code=403, detail="자신보다 높은 권한의 게시글은 삭제할 수 없습니다.")
            elif origin_write.mb_id and not is_owner(self.write, member_id):
                raise ClassException(status_code=403, detail="자신의 게시글만 삭제할 수 있습니다.", )

            if isinstance(ClassException, AlertException):
                ...
                if not origin_write.mb_id and not self.request.session.get(f"ss_delete_{self.bo_table}_{origin_write.wr_id}"):
                    ...
                    raise ClassException(status_code=403, detail="비회원 글을 삭제할 권한이 없습니다.", url=set_url_query_params(url, query_params))
            else:
                if not origin_write.mb_id:
                    raise ClassException(status_code=403, detail="비회원 글을 삭제할 권한이 없습니다.")
